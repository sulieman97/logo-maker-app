...
const App = () => {
  ...
  const analyzeAndGenerate = async () => {
    ...
    const analysisPrompt = `
      ...
      Return ONLY a JSON object:
      {
        ...
        "variants": [
          {"id": 1, "title": "تصميم عصري بسيط", "prompt": "Professional minimalist logo for '${projectName}', clean lines, white background"},
          {"id": 2, "title": "تصميم إبداعي فاخر", "prompt": "Luxurious creative logo for '${projectName}', elegant details, high contrast, white background"}
        ],
    ...
    try {
      ...
      setError("فشل في معالجة البيانات. تأكد من جودة الاتصال.");
      console.error(err);
    } finally {
      setIsGenerating(false);
    }
  ...
  cconst generateImage = async (prompt, index) => {
    ...
    try {
      // 2. تجهيز الوصف (Prompt) وإضافة لمسات التصميم
      const professionalPrompt = `${prompt}, professional vector logo, flat design, white background, high resolution, minimalist`;
      const encodedPrompt = encodeURIComponent(professionalPrompt);
      
      // 3. إنشاء رابط Pollinations (بدون الحاجة لـ API Key)
      ...
      // 4. تحديث الرابط فوراً في المصفوفة
      setImages(prev => {
        const updated = [...prev];
      ...
      // 5. محاكاة انتظار بسيط لضمان أن المتصفح بدأ بطلب الصورة ثم إيقاف حالة التحميل
      setTimeout(() => {
        setIsGeneratingImages(prev => {
          const updated = [...prev];
          updated[index] = false;
      ...
      }, 1000); // ننتظر ثانية واحدة فقط ليظهر الـ Placeholder ثم الصورة
                  ...
                  <div className="aspect-square bg-slate-50 rounded-[2.5rem] overflow-hidden border border-slate-100 relative mb-5 flex items-center justify-center shadow-inner">
                    {isGeneratingImages[idx] || (isGenerating && !results) ? (
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Generating Concept...</span>
                      </div>
                    ) : images[idx] ? (
